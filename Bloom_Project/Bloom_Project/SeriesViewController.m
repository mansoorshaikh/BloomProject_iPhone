...
@synthesize mvc,activityIndicator,SeriesTableView,seriesArray,appDelegate,overlayView,isMenuVisible,menuNameArray,menuNameButton,navigationmenuBtn,navigationSearchBtn,bloomimg;
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
        [SeriesTableView removeFromSuperview];
        SeriesTableView=[[UITableView alloc]initWithFrame:CGRectMake(0,180,self.view.bounds.size.width,self.view.bounds.size.height-430)];
        SeriesTableView.backgroundColor=[UIColor clearColor];
        self.SeriesTableView.separatorColor = [UIColor clearColor];
        [self.view addSubview:SeriesTableView];
        [self.view bringSubviewToFront:SeriesTableView];
    ...
    if(height>=480 && height<568){
        ...
        [SeriesTableView removeFromSuperview];
        SeriesTableView=[[UITableView alloc]initWithFrame:CGRectMake(0,125,self.view.bounds.size.width,self.view.bounds.size.height-285)];
        SeriesTableView.backgroundColor=[UIColor clearColor];
        self.SeriesTableView.separatorColor = [UIColor clearColor];
        [self.view addSubview:SeriesTableView];
        [self.view bringSubviewToFront:SeriesTableView];
...
-(void)getSeriesData{
    [NSThread detachNewThreadSelector:@selector(threadStartAnimating:) toTarget:self withObject:nil];
        NSString *urlString = [[NSString alloc]initWithFormat:@"http://app.bloomproject.com/api/v1/getVideo.php?tag=series,primary video"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SeriesVO *serVO=[seriesArray objectAtIndex:indexPath.row];
