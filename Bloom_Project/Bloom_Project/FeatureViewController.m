...
- (void)viewDidLoad {
    ...
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
        [featureTableView removeFromSuperview];
        featureTableView=[[UITableView alloc]initWithFrame:CGRectMake(0,110,self.view.bounds.size.width,self.view.bounds.size.height-360)];
        featureTableView.backgroundColor=[UIColor clearColor];
        self.featureTableView.separatorColor = [UIColor clearColor];
        [self.view addSubview:featureTableView];
        [self.view bringSubviewToFront:featureTableView];
    ...
    if(height>=480 && height<568){
        ...
        [featureTableView removeFromSuperview];
        featureTableView=[[UITableView alloc]initWithFrame:CGRectMake(0,110,self.view.bounds.size.width,self.view.bounds.size.height-265)];
        featureTableView.backgroundColor=[UIColor clearColor];
        self.featureTableView.separatorColor = [UIColor clearColor];
        [self.view addSubview:featureTableView];
        [self.view bringSubviewToFront:featureTableView];
...
-(void)menuButtonAction:(UIButton *)button{
    ...
    if (tags==0) {
        ...
        if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
            setting = [[SettingViewController alloc] initWithNibName:@"RegisterViewController~ipad" bundle:nil];
...
-(void)getFeatureData{
    ...
    NSString *urlString = [[NSString alloc]initWithFormat:@"http://app.bloomproject.com/api/v1/getVideo.php?tag=white_water_rafting"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SeriesVO *serVO=[featureArray objectAtIndex:indexPath.row];
